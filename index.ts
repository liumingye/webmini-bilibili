...
export class extension implements PluginMinimalData {
  ...
  userAgent = {
    desktop: ["passport.bilibili.com/login"],
    mobile: [
      "m.bilibili.com/",
    ...
    if (["www.bilibili.com", "m.bilibili.com"].includes(url.hostname)) {
      ...
      if (vid) {
        if (url.hostname === "m.bilibili.com") {
          ...
          if (vid !== this.last.vid) {
            getPartOfVideo(this.api.application, this.api.net, vid);
            this.last.vid = vid;
            this.api.application.mainWindow?.send(
              "setAppState",
              "disableDanmakuButton",
            ...
            this.api.application.selectPartWindow?.send(
      ...
      if (bvid) {
        if (url.hostname === "m.bilibili.com") {
          ...
          getPartOfBangumi(this.api.application, this.api.net, bvid);
          this.api.application.mainWindow?.send(
            "setAppState",
            "disableDanmakuButton",
            false
